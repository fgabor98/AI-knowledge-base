...
This page covers how readiness, nonblocking I/O, edge triggering, fairness, and descriptor ownership shape an event loop. It is part of Stage 7: IPC And Event-Driven Design and focuses on behavior that must remain correct on a constrained or partially available embedded Linux target.

## Core Concepts

- the event loops: select, poll, and epoll contract;
...
- mental model and vocabulary for event loops: select, poll, and epoll;
...
build an event loop with bounded work, explicit rearming, timeout, and shutdown behavior.
