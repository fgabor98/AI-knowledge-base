...
This page covers how applications behave when the system image is immutable and only selected paths are writable. It is part of Stage 1: Linux Runtime, Filesystem, And Rootfs and focuses on behavior that must remain correct on a constrained or partially available embedded Linux target.

## Core Concepts

- the read-only rootfs, overlayfs, and persistent state contract;
...
- mental model and vocabulary for read-only rootfs, overlayfs, and persistent state;
...
classify an application’s files into image, configuration, runtime, cache, log, and persistent data.
