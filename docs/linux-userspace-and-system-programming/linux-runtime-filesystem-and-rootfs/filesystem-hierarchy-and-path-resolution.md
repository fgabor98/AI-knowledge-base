...
This page covers how Linux names files, resolves paths, and exposes system state through standard directories. It is part of Stage 1: Linux Runtime, Filesystem, And Rootfs and focuses on behavior that must remain correct on a constrained or partially available embedded Linux target.

## Core Concepts

- the filesystem hierarchy and path resolution contract;
...
- mental model and vocabulary for filesystem hierarchy and path resolution;
...
inspect a target rootfs and explain the purpose and mutability of its major directories.
